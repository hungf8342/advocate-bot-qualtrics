"""Consumer-debt field export specs and row building."""
...
from advocate_bot_qualtrics.core.export_writer import append_row
from advocate_bot_qualtrics.core.session import NodeAnswerRecord
from advocate_bot_qualtrics.practice_areas.consumer_debt.session import InteractiveSessionState

CORRECTION_CONFIDENCE_CAP = 69

_FIELD_SPECS: tuple[tuple[str, str, tuple[str, ...], str], ...] = (
    (
        "filing_date",
        "Date claim was filed",
        ("get_filing_date", "different_complaint_filing", "confirm_filing_date"),
        "filing_date_changed",
    ),
    (
    ...
    (
        ...
        ("get_last_payment_OG_creditor", "additional_payment_OG_creditor"),
        "",
    ),
    (
        "last_payment_debt_collector",
        "Last payment to debt collector",
        ("get_last_payment_debt_collector", "additional_payment_debt_collector"),
        "",
    ),
)
...
def build_field_confidence_rows(session: InteractiveSessionState) -> list[FieldConfidenceRow]:
...
def append_session_fields_row(
    ...
    *,
    ...
    return append_row(path, XLSX_HEADERS, row)
